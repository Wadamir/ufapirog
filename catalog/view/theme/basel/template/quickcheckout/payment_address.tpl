...
<div id="payment-new" style="display: <?php echo ($addresses ? 'none' : 'block'); ?>;">
	<?php foreach ($fields as $field) { ?>
		...
		<?php } else { ?>
			<?php if (!empty(${'field_' . $field}['display'])) { ?>
				<div <?php echo $field == 'postcode' ? ' id="payment-postcode-required"' : ''; ?> class="tesdt col-sm-6 <?php echo !empty(${'field_' . $field}['required']) ? ' required' : ''; ?>">
					<label class="control-label" for="input-payment-<?php echo str_replace('_', '-', $field); ?>"><?php echo ${'entry_' . $field}; ?></label>
					<input type="text" name="<?php echo $field; ?>" placeholder="<?php echo !empty(${'field_' . $field}['placeholder']) ? ${'field_' . $field}['placeholder'] : ''; ?>" value="<?php echo ${'field_' . $field}['default']; ?>" class="form-control <?php echo ($field == 'city') ? ' disabled' : ''; ?>"  id="input-payment-<?php echo str_replace('_', '-', $field); ?>" <?php echo ($field == 'city') ? 'disabled="disabled"' : ''; ?> />
				</div>
			<?php } else { ?>
