<?php foreach ($fields as $field) { ?>
  ...
  <?php } else { ?>
	...
	<div <?php echo $field == 'postcode' ? ' id="payment-postcode-required"' : ''; ?> class="col-sm-6<?php echo !empty(${'field_' . $field}['required']) ? ' required' : ''; ?>">
	  ...
	  <input type="text" name="<?php echo $field; ?>" placeholder="<?php echo !empty(${'field_' . $field}['placeholder']) ? ${'field_' . $field}['placeholder'] : ''; ?>" value="<?php echo ${$field} ? ${$field} : ${'field_' . $field}['default']; ?>" class="form-control<?php echo ($field == 'city') ? ' disabled' : ''; ?>"  id="input-payment-<?php echo str_replace('_', '-', $field); ?>" <?php echo ($field == 'city') ? 'disabled="disabled"' : ''; ?> />
	</div>
	<?php } else { ?>
...
<div class="labels-holder hidden">
