...
$(document).ready(function () {
	...
	var top_line_height = $(".top-line").outerHeight() || 0;

	var stickyHeader = function () {
		var window_to_top = $(window).scrollTop();
		if (window_to_top > sticky_to_top + sticky_header_height + 40) {
			$("body").css("padding-top", sticky_header_height + "px");
		...
		if (window_to_top > sticky_to_top + 50) {
...
var cart = {
	add: function (product_id, quantity, source) {
		...
		// var product_wrapper = 'product-' + product_id;
		var product_options = $(
			"#product-" +
				product_id +
				" input[type='text'], #product-" +
				product_id +
				" input[type='number'], #product-" +
				product_id +
		...
		$.ajax({
			...
			// data: 'product_id=' + product_id + '&quantity=' + (typeof(quantity) != 'undefined' ? quantity : 1) + '&options=' + product_options,
			dataType: "json",
	...
	remove: function (key) {
		...
		$.ajax({
			...
			success: function (json) {
				console.log("success remove from cart!" + json);
			...
			error: function (xhr, ajaxOptions, thrownError) {
				...
				if (xhr.responseText) {
					message +=
						"Response: " + xhr.responseText.substring(0, 200); // cut to avoid huge HTML dump
				}
...
/* Live options */
var price_with_options_ajax_call2 = function (product_id_wrapper) {
	var product_id = $(
		"#" + product_id_wrapper + " input[name='product_id']",
	).val();
	console.log(
		$("#" + product_id_wrapper + " input[type='radio']:checked").val(),
	);
	$.ajax({
		type: "POST",
		url:
			"index.php?route=extension/basel/live_options/index&product_id=" +
			product_id,
		data: $(
			"#" +
				product_id_wrapper +
		...
		success: function (json) {
			...
			if (json.success) {
				if (
					$("#" + product_id_wrapper + " .price-new").length > 0 &&
					json.new_price.special
				) {
					animation_on_change_price_with_options(
						"#" + product_id_wrapper + " .price-new",
						json.new_price.special,
					);
				}
				if (
					$("#" + product_id_wrapper + " .price-regular").length >
						0 &&
					json.new_price.price
				) {
					animation_on_change_price_with_options(
						"#" + product_id_wrapper + " .price-regular",
						json.new_price.price,
					);
				}
			}
...
var animation_on_change_price_with_options = function (
	selector_class_or_id,
	new_html_content,
) {
...
$(document).on(
	"change",
	".product-options input[type='text'], .product-options input[type='number'], .product-options input[type='hidden'], .product-options input[type='radio']:checked, .product-options input[type='checkbox'], .product-options select, .product-options textarea, .product-options input[name='quantity']",
	function () {
		var product_id_wrapper = $(this).closest(".single-product").attr("id");
		console.log(product_id_wrapper);
		price_with_options_ajax_call2(product_id_wrapper);
	},
);
/* Live options */
