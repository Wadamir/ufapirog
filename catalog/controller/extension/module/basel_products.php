            ...
            if (in_array($key, $setting['selected_tabs']['tabs'])) {
                ...
                if (isset($results)) {
                    foreach ($results as $result) {
                        ...
                        if (strlen($result['description']) < $this->config->get($this->config->get('config_theme') . '_product_description_length')) {
                            $product_description = html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8');
                        } else {
                            $product_description = utf8_substr(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'), 0, $this->config->get($this->config->get('config_theme') . '_product_description_length'));
                        }

                        /*options in category */
                        $var_currency = array();
                        $var_currency['currency_value'] = $this->currency->getValue($this->session->data['currency']);
                        ...
                        /*options in category end */

                        $products[] = array(
                            'options'             => $options, // options in category
                            'price_no_format'     => $result['price'], // options in category
                            'special_no_format' => $result['special'], // options in category					
